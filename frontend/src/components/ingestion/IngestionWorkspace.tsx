  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 rounded-2xl bg-slate-900/60 border border-slate-800/80 shadow-lg">
        <div>
          <h1 className="text-xl font-bold text-slate-100">Statement Ingestion Engine</h1>
          <p className="text-xs text-slate-400 mt-1">
            Upload CSV bank exports, PDF statements, or OFX/QFX downloads with intelligent deduplication and auto-categorization.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <select
            ...
            className="px-3.5 py-2 bg-slate-800 border border-slate-700 text-slate-200 rounded-lg text-xs font-medium focus:ring-2 focus:ring-indigo-500 focus:outline-none"
          >
            {accounts.map((acc) => (
              <option key={acc.id} value={acc.id}>
                {acc.name} ({acc.institution} • ${acc.current_balance.toLocaleString()})
              </option>
            ))}
          </select>
        </div>
      ...
      {!preview && (
        <div
          ...
          className={`relative cursor-pointer border-2 border-dashed rounded-3xl p-12 text-center transition-all duration-200 ${
            isDragging
        ...
        >
          ...
          <div className="flex flex-col items-center justify-center space-y-4">
            <div className="w-16 h-16 rounded-2xl bg-indigo-600/15 border border-indigo-500/30 flex items-center justify-center text-indigo-400 shadow-xl shadow-indigo-500/10">
              {isLoading ? (
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-400">
                ...
                <span className="text-indigo-400 font-medium">PDF Statements</span> (Chase, BoA, Amex, Credit Unions), and{" "}
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 p-4 rounded-xl bg-slate-900/80 border border-slate-800">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3 text-xs">
                <span className="text-slate-400">
                  Total: <b className="text-slate-200">{preview.total_parsed}</b>
                </span>
                <span className="text-emerald-400">
                  New: <b>{preview.new_count}</b>
                </span>
                {preview.duplicates_count > 0 && (
                  <span className="text-amber-400">
                    Duplicates: <b>{preview.duplicates_count}</b>
                  </span>
                )}
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="px-3.5 py-1.5 rounded-lg border border-slate-700 text-slate-400 hover:text-slate-200 text-xs font-medium"
              >
                Cancel
              </button>
              <button
              ...
              >
                {isCommitting ? (
                  ...
                  <>
                    Commit {selectedHashes.size} Transactions <ArrowRight className="w-4 h-4" />
                  </>
                )}
              </button>
            </div>
