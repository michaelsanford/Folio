    ...
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-black/75 backdrop-blur-xs animate-in fade-in">
      <div className="w-full max-w-xl rounded-2xl bg-slate-900 border border-slate-800 shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-5 overflow-y-auto space-y-5 text-xs">
          {/* Live Card Preview */}
          <div>
            <label className="block text-slate-400 font-semibold uppercase tracking-wider text-[10px] mb-2">
              Live Preview
            </label>
              ...
              {COLOR_SWATCHES.map((swatch: ColorSwatch) => {
                ...
                return (
                  <button
                    ...
                    className={`h-8 rounded-xl flex items-center justify-center transition-transform hover:scale-105 border ${
                      isSelected
          ...
          <div>
            ...
            {/* Category Tabs */}
            <div className="flex gap-1 overflow-x-auto pb-1.5 mb-3 no-scrollbar">
              {categories.map((c) => (
                <button
                  key={c.id}
                  type="button"
            ...
            {/* Icons Grid */}
            <div className="grid grid-cols-4 sm:grid-cols-7 gap-2 max-h-56 overflow-y-auto p-1 pr-2">
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-950/40 flex items-center justify-end gap-2">
          ...
          <button
            type="button"
            onClick={handleApply}
            className="px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-semibold text-xs shadow-lg shadow-indigo-500/20 transition-all"
          >
            Apply Glyph & Color
          </button>
        </div>
