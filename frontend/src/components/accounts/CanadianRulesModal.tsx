    ...
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-black/75 backdrop-blur-xs animate-in fade-in">
      <div className="w-full max-w-xl rounded-2xl bg-slate-900 border border-slate-800 shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto space-y-6 text-sm">
          ...
          <div>
            ...
            <div className="space-y-2.5">
              {meta.keyRules.map((rule, idx) => (
                <div
                  key={idx}
                  className="flex items-start gap-3 p-3.5 rounded-xl bg-slate-800/40 border border-slate-700/40 text-slate-200 text-sm"
                >
          ...
          {meta.craUrl && (
            <div className="p-4 rounded-xl bg-indigo-950/20 border border-indigo-800/40 flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <Info className="w-5 h-5 text-indigo-400 shrink-0" />
                <div>
                  <div className="font-semibold text-slate-200 text-sm">Official Government of Canada Guide</div>
              ...
              <a
                ...
                className="flex items-center gap-1.5 px-3.5 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white font-semibold text-xs shadow-md transition-colors shrink-0"
              >
                <span>Canada.ca</span>
